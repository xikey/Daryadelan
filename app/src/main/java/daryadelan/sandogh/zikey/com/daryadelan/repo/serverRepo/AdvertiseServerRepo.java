...
public class AdvertiseServerRepo implements IAdvertise {
    ...
    @Override
    public void getGallary(Context context, String name, String tokenType, String token, IRepoCallBack<AdvertiseWrapper> callBack) {

        IAdvertiseApi advertiseApi = ServerApiClient.getClientWithHeader(context, tokenType, token).create(IAdvertiseApi.class);
        call = advertiseApi.getGallery(name);
        call.enqueue(new Callback<AdvertiseWrapper>() {
            @Override
            public void onResponse(Call<AdvertiseWrapper> call, Response<AdvertiseWrapper> response) {
                if (response == null) {
